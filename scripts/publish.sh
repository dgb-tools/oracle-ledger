...
echo "== deploy"; ( cd "$SITE/.." && CLOUDFLARE_API_TOKEN=$(cat ~/.cf_dgbinsights_deploy_token) npx --yes wrangler@3 pages deploy site --project-name dgbinsights --commit-dirty=true 2>&1 | grep -E "Deployment complete|rror" )
